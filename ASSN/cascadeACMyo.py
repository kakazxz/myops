'''
this multimodality segmentation network with autoencoder
'''
import os
import scipy.misc
import numpy as np
...
flags.DEFINE_string("dataset_dir", "../datasets/ACMyo", "Dataset directory.")
flags.DEFINE_string("checkpoint_dir", "../outputs/ACMyo/checkpoint", "Directory name to save the checkpoints [checkpoint]")
flags.DEFINE_string("sample_dir", "../outputs/ACMyo/samples", "Directory name to save the image samples [samples]")
flags.DEFINE_string("test_dir", "../outputs/ACMyo/test", "Directory name to save the image samples [samples]")
flags.DEFINE_string("valid_dir", "../outputs/ACMyo/valid", "Directory name to save the image samples [samples]")
flags.DEFINE_string("log_dir", "../outputs/ACMyo/logs", "Directory name to save the logs [logs]")
flags.DEFINE_string("results_dir", "../outputs/ACMyo/logs", "test result")
...
flags.DEFINE_float("ce_reg", 1, "number of label")
flags.DEFINE_float("ac_reg", 0.1, "number of label")
flags.DEFINE_float("dice_reg", 0, "number of label")
...
flags.DEFINE_string("phase", "valid", "train or test")
...
with tf.Session(config=config) as sess:
    if FLAGS.phase=="train":
        ...
        model = ACMyoMultiSeq(sess, config=FLAGS, train=True)
        model.train(FLAGS)
    elif  FLAGS.phase=="test":
        prepare_test_data(FLAGS)
        mk_or_cleardir(FLAGS.test_dir)
        model = ACMyoMultiSeq(sess, config=FLAGS, train=False)
        model.test()
    else:
